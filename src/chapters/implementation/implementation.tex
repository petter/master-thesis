...
I chose to structure my tests into separate files under the \codeword{__test__} folder.
...
Listing~\vref{lst:ava-test} shows one of the tests from the source code of the compiler, where we similarily to the tree-sitter test we saw before, check that a closed template is working as expected.
...
In order to finish the compiler we would have to implement the \codeword{addto}-statement properly, and be able to properly handle all declarations and references, so that these can be renamed as expected.
