\subsection{Implementing PT as a TypeScript Library}\label{subsec:implementing-pt-as-a-ts-library}
...
%This would entail creating an internal DSL\footnote{Internal DSLs are domain specific languages that can be used inside} for defining templates and a library for achieving the functionality of PT, such as instantiating, renaming and class merging.
%This approach would presumably be an easier task than having to create a compiler, and it could potentially make it easier to use, as the programmer would not have to install a compiler in order to get the PT functionality.
