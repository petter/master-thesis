...
Since we made the classes static attributes of the template class we could easily just create a new static field on the template class (with declaration merging to get the correct type for the new name) and \codeword{delete}\cite{deleteop} the old field.
