...
What this means is that after performing all instantiations, with optional renamings, we can momentarily end up with several class declaration, which would be an invalid TypeScript program, however before the closing of templates step is done we will merge all class declarations and \codeword{addto}-statements.
