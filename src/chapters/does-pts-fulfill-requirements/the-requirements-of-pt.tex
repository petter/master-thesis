...
If we have a template with classes \codeword{A} and \codeword{B}, and class \codeword{B} a subclass of class \codeword{A}, then this relation should not be affected by any additions or merges done to either of the classes.
...
At instantiation of the \codeword{ListsOf} template we can give i.e. a \codeword{Person} class, containing some information about a persons name, date of birth, etc., as the actual parameter and then we would not have to keep respecifying the actual parameter for every reference.
...
If we merge two classes \codeword{A} and \codeword{B}, it should be possible to reach all \codeword{B} attributes from an \codeword{A}-variable, and vice versa.
