...
% TODO: Uklart hva dette betyr
% The extension of classes using the \codeword{addto}-clause is done independently for each class, ignoring any inheritance-relations.
...
Any pointers typed with the old \codeword{A} or \codeword{B} will now be typed as the new merged class \codeword{C}.
...
Here \codeword{R} is a required type which can be substituted for any class or interface at instantiation.
...
We could then instantiate the template, \codeword{T}, giving class or interface pointers as actual parameters for the required types, as seen below.
...
An \textit{open template} on the other hand is a template which has one or more instantiations or \codeword{addto}-statements in its body.
