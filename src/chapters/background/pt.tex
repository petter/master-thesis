...
Instantiation is done inside the body of a package/template with the use of a \codeword{inst}-clause.
...
If an addition has the same signature as an already existing field from the instantiated template class, then the addition will override the already existing field, like in traditional inheritance.
...
If two classes don't share the same name, it is still possible to merge them by through renaming.
Through renaming it is possible to force merging by giving the classes the same name.
In listing~\vref{code:renameclassmerging} we see an example of renaming class \codeword{B} to \codeword{A} to force these classes to be merged under the class name \codeword{A}.
Merging the classes would simply lead to having a single class \codeword{A} with the field from both classes.
